...
extern MY_HIGHP_OR_MEDIUMP vec2 inverse_booster;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    MY_HIGHP_OR_MEDIUMP number fac2 = 0.5 + 0.5*sin(10.*uv.x+2.32*uv.y + inverse_booster.r*5. - cos(inverse_booster.r*2.3 + uv.x*8.2));
    MY_HIGHP_OR_MEDIUMP number fac3 = 0.5 + 0.5*sin(12.*uv.x+6.32*uv.y + inverse_booster.r*6.111 + sin(inverse_booster.r*5.3 + uv.y*3.2));
    MY_HIGHP_OR_MEDIUMP number fac4 = 0.5 + 0.5*sin(4.*uv.x+2.32*uv.y + inverse_booster.r*8.111 + sin(inverse_booster.r*1.3 + uv.y*13.2));
    MY_HIGHP_OR_MEDIUMP number fac5 = sin(0.5*16.*uv.x+5.32*uv.y + inverse_booster.r*12. + cos(inverse_booster.r*5.3 + uv.y*4.2 - uv.x*4.));
